...
export async function getProfileDetailsApi(mobile_no: any): Promise<any> {
    try {
        const response: AxiosResponse<any> = await apiService.get(`/get-profile-details?mobile_no=${mobile_no}`);
        ...
        if (error.response) {
            ...
            console.error("getProfileDetailsApi:: Server Response Error:", error.response.data);
            ...
            console.error("getProfileDetailsApi:: Request Setup Error:", error.message);
...
export async function getUserDetailsApi(mobile_no: any): Promise<any> {
    try {
        const response: AxiosResponse<any> = await apiService.get(`/get-user?mobile_no=${mobile_no}`);
        console.log("getUserDetailsApi:: Response:", response.data.user);
        return response.data;
    } catch (error: any) {
        ...
        if (error.response) {
            // The request was made and the server responded with a status code
            // that falls out of the range of 2xx
            console.error("getUserDetailsApi:: Server Response Error:", error.response.data);
            throw new Error(`Server error: ${error.response.data.message || 'An error occurred'}`);
        } else if (error.request) {
            // The request was made but no response was received
