...
import apiServiceLocal from "./Api_Local";
...
export async function getDayEventsApi(date, obj) {
    console.log("getDayEventsApi::", date, obj)
    try {
        const response = await apiService.get(`/get-particular-date-events?mobile_no=${obj.mobile_no}&date=${date}`,);
...
export async function getMonthEventsApi(obj: any) {
    ...
    try {
        const response = await apiService.get(`/get-events-by-month/${obj.mobile_no}/${obj.month}`);
...
export async function fetchZohoBalanceApi(customer_id: string) {
    console.log("apiapiapi::", customer_id);
    try {
        const response = await apiService.get(`/get-zoho-customer?customer_id=${customer_id}`);
        console.log("fetchData RESSS::::", response.data);
        return response.data;
    } catch (error) {
...
export async function getZohoCustomerId(mobile_no: number) {
    try {
        const response = await apiService.get(`/get-zoho-customer-id?mobile_no=${mobile_no}`);
        if (response.status == 200)
            return response.data.zoho_customer_id;
...
export async function getZohoCustomerTransaction(id: any) {
    try {
        const response = await apiService.get(`/get-zoho-transactions?customer_id=${id}`);
        return response.data;
    } catch (error) {
...
export async function getShopProductsApi(type: string, tagName: string, priceRange: number, region: string, page: number) {
    try {
        let url = `/api/productss?productType=${type}&page=${page}`;
        if (tagName) {
        ...
        // return response.data;
        console.log("response=======responseresponseresponseresponseresponseresponse===", response)
